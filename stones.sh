...
draw_pyramid() {
  ...
  local k=1
  while (( (k+1)*(k+2)/2 <= n )); do
    ((k++))
  done

  # Try base = k+1 if possible
  local base=$k
  if (( n >= (k+1)*k/2 )); then
    base=$((k+1))
  fi

  # Greedily fill from bottom up
  local rows=()
  local stones_left=$n
  local row_len=$base
  while ((stones_left > 0 && row_len > 0)); do
    if ((stones_left >= row_len)); then
      rows=($row_len "${rows[@]}")
      stones_left=$((stones_left - row_len))
      ((row_len--))
    else
      rows=($stones_left "${rows[@]}")
      stones_left=0
    fi
  done
  ...
  for ((i=0; i<num_rows; i++)); do
    rowlen=${rows[i]}
    # For brick effect, offset by even multiples
    pad=$(( (num_rows-i-1)*2 ))
    for ((j=0; j<pad; j++)); do printf " "; done
    for ((j=0; j<rowlen; j++)); do printf "%s" "$symbol"; done
    printf "\n"
  done
}
